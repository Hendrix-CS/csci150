% -*- compile-command: "pdflatex class-notes-S19.tex" -*-
...
\title{CSCI 150: Foundations of Computer Science \\ Spring 2019 Lecture
  Notes}

\maketitle

\section{\notready Introduction (W 1/16)}

\begin{reflect}
  Want to look at that article on how to start first class, maybe
  revise a bit.  Also decide/tell them about required meeting with me?
\end{reflect}
...
XXX maybe have them talk to each other about clubs/sports/activities
they are in.  How do you improve at that?  How do you think you
...
\newpage
\section*{\ready LAB: Minecraft Hour of Code (1/16, 1/17 )}

\newpage
\section{\ready Algorithms and language (F 1/18)}
...
\textbf{Note}: quiz Friday on basic Python!  You will have a chance to
  practice on Weds/Thurs in lab.  There isn't a textbook on purpose,
  to force you to play/try things instead of using the textbook as a
  crutch.  Install PyCharm or use repl.it, play around.  No substitute
...
\section{\ready Python II, Box Volume (W 1/23)}
...
\begin{itemize}
\item \ready Bring color printouts of Minecraft world.
\end{itemize}
...
\section*{\ready LAB: Kepler \& Newton (2/23--24)}
...
\newpage
\section{\ready Tracing I (F 1/25)}

\subsection*{Setup}
\begin{itemize}
\item \ready \textbf{Quiz (Python syntax and arithmetic)}.  See
  ...
  confer with other pairs, \etc)
\item \ready HW: tracing practice (on website)
\end{itemize}
...
\section{\ready Booleans \& Conditionals (M 1/28)}
...
\section*{\notready More conditionals, conditional practice (W 1/30)}
...
\textcolor{red}{Get them to practice some things that will get them ready for
  doing the lab.  I.e. provide some scenarios that they have to encode
  somehow?  Nested ifs, ...}

\newpage
\section*{\notready LAB: Big Data/Heart Disease (Th 9/6)}

\textcolor{red}{get improved graphs from Mark}
...
\section{\ready Information encoding I (F 2/1)}
...
\item \ready HW due Monday (conditional practice).
...
\item \ready \textbf{Assign project 1}---due Friday 9/21.  You might decide
...
% There was one thing I didn't quite get to at the end of class today.  It's not hard to explain, but important.
...
\newpage
\section{\ready While loops (M 2/11)}

\subsection*{Setup}
\begin{itemize}
\item Reminder: Project 1 due 1 week from today!
\end{itemize}
...
\newpage
\section{\notready Functions II: call stack, return values (F 2/15)}

% To do: plan out formal presentation of tracing with stack diagrams
...
\newpage
\section{\ready Functions III: Abstraction (M 2/18)}

\subsection*{Setup}
\begin{itemize}
\item Reminder: HW due Wednesday with quiz: functions (tracing with
  stack)
\item Project 1 due today.
\end{itemize}
...
\newpage
\section*{\notready LAB: Mutation is the Word (Doublets) (2/20--21)}

% TODO: refactor lab along the lines Gabe suggests below.
...
\newpage
\section{\notready More string, loop, \& function practice (F 2/22)}

% TODO: finish lab above; decide re: DNA strings HW? Something else?
% Make turnin thing for DNA strings HW; update HW to specify turning
% it in electronically.
%
% Lecture notes are otherwise ready.
...
\newpage
\section{\notready Exam 2 review (M 10/8)}

Spend a lecture going over practice exam.

\section{\notready Exam 2 (W 10/10)}
...
\newpage
\section{\notready More for loops: index ranges (W 10/17)}
...
\section*{\notready LAB: Caesar's Secrets (Th 10/18)}
...
\section{\notready Mutability, objects, and references (F 10/19)}
...
\newpage
\section*{\notready LAB: Sentiment Analysis (W 10/24, Th 10/25)}
...
\newpage
\section{\notready Heap tracing examples, dictionary practice (F 10/26)}
...
\newpage
\section{\notready Recursion I: intro (M 11/5)}
...
\newpage
\section{\notready Recursion II: recursive functions (W 11/7)}
...
\newpage
\section*{\notready LAB: Fractal Recursion (Th 11/8)}
