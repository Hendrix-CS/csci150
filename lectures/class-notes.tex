...
Ada Lovelace: 1815-1852, first programmer.  Wrote programs for Charles
Babbage's Analytical Engine (never built, but definitely works in
principle).  Babbage's purpose for the AE was limited to making tables
of numbers, but Lovelace had a much more expansive and far-seeing
vision:
...
information instead of just making tables of numbers (what Babbage
wanted to do).  Over the next 2 classes we will consider some of the
fundamentals that make this possible.
...
Integers are represented like this inside the computer.  Most modern
computers use $64$ bits to represent an integer (\emph{how big is
  that?}). What about negative numbers?  One possibility: use one bit
for sign (plus or minus).  (Turns out there is a better way, ``$2$'s
complement''; learn about it in CSO or ask if you are curious.)

\subsection*{Representing floating-point numbers}

\emph{Might only get partway through this; just pick up next class\dots}
