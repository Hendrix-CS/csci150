...
\begin{itemize}
\item Book talks about 5 aspects of algorithms: input, output, math,
  conditionals, repetition.  Where did we see them in Minecraft?
...
\item For this week, read Chapters 2 and 3. ???
\end{itemize}
...
\end{verbatim}
  \end{enumerate}
\item HW: ???
\item Start reading Chapter 4. XXX
\item Hand out copies of first tracing template.
\end{itemize}
...
XXX make tracing template and some exercises to do in class.  Have
them make exercises for each other.
...
\item XXX NEED A QUIZ, which one? Predict output of a program with
  conditionals.
  % XXX Quiz: program with conditionals, ask them to identify five
  % errors.
\end{itemize}
...
\newpage
\section{Execution tracing with a function stack (F 9/28)}

\textbf{Brent is gone at ICFP}

\subsection*{Setup}
\begin{itemize}
\item XXX Reminder: DNA Strings HW due Wednesday before class.
\item XXX Reminder: quiz next class on strings.
\end{itemize}

XXX TRACING WITH FUNCTION STACK.  Need to rewrite this lecture.
The practice will come from tracing some functions that use these
features (strings, loops, functions).

Stuff from before (maybe it can be used):

Do version of \verb|int_input| that accepts negative numbers as well
as positive (uses indexing, slicing, \verb|or|)?
...
\section*{LAB: Caesar's Secrets (Th 10/4)}
...
\newpage
\section{For loop practice (F 10/5)}
