...
def float_input() -> float:
    ...
    while not success:
        ...
        if ans.isdigit():
            ...
            place = ans.find('.')
            if ans[place + 1:].isdigit():
                if ans[0:place].isdigit() or len(ans[0:place]) > 0 and ans[0] == '-' and ans[1:place].isdigit():
                    return float(ans)
...
## Given a list of ints, count how many are even
def even_list(lst: List[int]) -> int:
    evens = 0
    i = 0
    while i < len(lst):
        if lst[i] % 2 == 0:
...
print(' ** %trh '.join(['Hi','there','how','are','you?']))
